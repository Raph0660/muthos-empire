export default function MentionsLegales() {
  return (
    <main className="max-w-2xl mx-auto py-20 px-6 font-serif">
      <h1 className="text-3xl font-bold mb-10">Mentions Légales</h1>
      <div className="space-y-8 text-stone-700">
        <p><strong>Éditeur :</strong> Muthos Labs, 12 chemin des gorges, 69570 dardilly, France </p>
        <p><strong>Hébergement :</strong> Vercel Inc., Walnut, CA 91789, USA.</p>
        <p><strong>Propriété :</strong> Le contenu de ce site est la propriété exclusive de Muthos Labs.</p>
      </div>
    </main>
  );
}
